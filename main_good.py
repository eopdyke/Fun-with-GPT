...
def reset_game_state(level, player_base, game_fsm):
    ...
    if game_fsm.state == GameState.DEFEAT or game_fsm.state == GameState.MAIN_MENU:
        ...
        food_level = player_base.food_level
        avialble_units = player_base.available_units
        player_base = PlayerBase(20, settings.SCREEN_HEIGHT / 2 - 40)
        print(str(player_base.food_rate))
        player_base.reset(base_health, base_food_rate, base_gold, food_level, avialble_units)
        print(str(player_base.food_rate))
    ...
    if not first_enemy_spawned:
        ...
        if (current_time - last_enemy_spawn_time) >= (enemy_base.spawn_rate * 1000):
            ...
            if enemy_base.deployed < enemy_base.units:
                ...
                enemy_units.append(EnemyUnit(enemy_spawn_x, enemy_spawn_y, level_details))
...
def evaluate_end_round(victory, game_fsm, player_base):
    ...
    if victory:
        ...
        if action == 'start':
            ...
            #game_running = False
            #print("The current level is: "+str(current_round.level))
            player_base = reset_game_state(1, player_base, game_fsm)
...
def game(game_fsm, player_base):
    ...
    while game_fsm.state == GameState.PLAYING:
        ...
        for friendly in friendly_units:
            # Assume the unit is not engaged initially
            friendly_engaged = False

            friendly_rect = pygame.Rect(friendly.x, friendly.y, friendly.width, friendly.height)
            if friendly.type == "archer":
                for enemy in enemy_units:
                    if ((friendly.x - enemy.x) ** 2 + (friendly.y - enemy.y) ** 2) ** 0.5 <= friendly.range:
                        # Enemy within range, engage and shoot
                        #print("enemy in range! Distance "+ str(((friendly.x - enemy.x) ** 2 + (friendly.y - enemy.y) ** 2) ** 0.5 ))
                        friendly.shoot_arrow(enemy, player_base, current_round, current_time, level_details)
                        friendly_engaged = True
                        for friendly in friendly_units:
                            print(str(friendly.type)+" can shoot")
                            if friendly.current_arrow is not None:
                                # Update and potentially draw the current arrow
                                friendly.current_arrow.update(dt)
                                if friendly.current_arrow.alive:
                        ...
                        for arrow in friendly.projectiles[:]:
                            arrow.update(dt)
                            arrow_rect = pygame.Rect(arrow.x, arrow.y, 5, 2)
                            for enemy in enemy_units:
                                enemy_rect = pygame.Rect(enemy.x, enemy.y, enemy.width, enemy.height)
                                if arrow_rect.colliderect(enemy_rect):
                                    enemy.take_damage(friendly.attack_power, player_base, current_round, level_details)
                                    arrow.alive = False
                            if arrow.alive:
                                    arrow.draw(screen)
                            else:
